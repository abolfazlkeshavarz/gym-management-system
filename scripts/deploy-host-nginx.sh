...
# a local-only port that project's container publishes (see APP_HTTP_PORT
# in .env / docker-compose.yml). This script only ever touches files
# specific to THIS project (named by domain, or prefixed "nosrati-"); it
# never edits or removes another project's vhost.
#
# What it does:
...
APP_HTTP_PORT="${APP_HTTP_PORT:-8091}"
...
cp deploy/nginx/security-headers.conf /etc/nginx/snippets/nosrati-security-headers.conf
...
echo "127.0.0.1:${APP_HTTP_PORT} (this project's app container)."
