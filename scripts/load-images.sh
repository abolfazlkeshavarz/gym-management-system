...
#   ./scripts/load-images.sh [path/to/nosrati-gym-image.tar.gz]
#   make load-images
#
...
BUNDLE="${1:-}"
if [[ -z "$BUNDLE" ]]; then
  for candidate in nosrati-gym-image.tar.gz dist/nosrati-gym-image.tar.gz; do
    if [[ -f "$candidate" ]]; then
      BUNDLE="$candidate"
      break
    fi
...
if [[ -z "$BUNDLE" || ! -f "$BUNDLE" ]]; then
  ...
  echo "    scp dist/nosrati-gym-image.tar.gz USER@THIS_SERVER:$(pwd)/" >&2
  exit 1
fi
...
for img in nosrati-gym:latest; do
...
echo ""
echo "Image loaded. Start the stack without building:"
